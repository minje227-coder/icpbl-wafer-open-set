...
def find_best_threshold_for_class(rows, class_idx: int, thresholds):
    ...
    for threshold in thresholds:
        ...
        for row in class_rows:
            accept = row["confidence"] >= float(threshold)
...
def find_best_classwise_thresholds_independent(rows, thresholds):
    class_thresholds = {}
    class_metrics = {}
...
def resolve_output_path(model_path: Path, output_json: Path | None, multiple_models: bool):
    auto_name = f"{model_path.stem}_classwise_threshold_independent_test_results.json"
...
def main():
    parser = argparse.ArgumentParser(
        description="Find class-wise thresholds independently on the validation split and apply them to the test split."
    )
    ...
    for idx, model_path in enumerate(model_paths, start=1):
        ...
        if args.disable_thresholding:
            ...
            threshold_info, class_threshold_metrics, val_threshold_metrics = find_best_classwise_thresholds_independent(val_rows, thresholds)
            ...
            print("best validation thresholds (independent class-wise search):")
            for class_idx, threshold in threshold_info.items():
                print(f"  - {PRED_LABELS[class_idx]}: {threshold:.4f}")
