...
def collect_split_samples(data_root: Path, split: str):
...
def apply_global_threshold(rows, threshold: float):
    thresholded_rows = []
    for row in rows:
        new_row = dict(row)
        pred_index = row["argmax_index"] if row["confidence"] >= threshold else UNKNOWN_PRED_INDEX
...
def apply_classwise_thresholds(rows, thresholds: dict[int, float]):
    thresholded_rows = []
    for row in rows:
        new_row = dict(row)
...
def find_best_classwise_thresholds(rows, thresholds):
    best_thresholds = None
    best_metrics = None

    for threshold_values in itertools.product(thresholds, repeat=len(PRED_LABELS)):
        class_thresholds = {class_idx: float(threshold_values[class_idx]) for class_idx in range(len(PRED_LABELS))}
        thresholded_rows = apply_classwise_thresholds(rows, class_thresholds)
        ...
        if (
            metrics["open_macro_f1"] == best_metrics["open_macro_f1"]
            and metrics["open_accuracy"] == best_metrics["open_accuracy"]
            and sum(class_thresholds.values()) < sum(best_thresholds.values())
        ):
            best_metrics = metrics
            best_thresholds = class_thresholds
...
def resolve_output_path(model_path: Path, output_json: Path | None, multiple_models: bool):
    auto_name = f"{model_path.stem}_classwise_threshold_test_results.json"
...
def main():
    parser = argparse.ArgumentParser(
        description="Find class-wise thresholds on the validation split and apply them to the test split."
    )
    ...
    parser.add_argument("--threshold", type=float, default=None, help="Optional single global threshold. If omitted, class-wise thresholds are searched on val.")
    parser.add_argument("--threshold-min", type=float, default=0.3, help="Minimum threshold value used for class-wise search.")
    parser.add_argument("--threshold-max", type=float, default=0.99, help="Maximum threshold value used for class-wise search.")
    parser.add_argument("--threshold-steps", type=int, default=100, help="Number of evenly spaced thresholds used for class-wise search.")
    parser.add_argument(
        "--disable-thresholding",
        action="store_true",
        help="Disable unknown rejection and evaluate plain argmax predictions.",
    )
    parser.add_argument(
        "--output-json",
        type=Path,
    ...
    val_samples, val_class_counts = collect_split_samples(data_root, "val")
    test_samples, test_class_counts = collect_split_samples(data_root, "test")
    ...
    for idx, model_path in enumerate(model_paths, start=1):
        ...
        val_rows = run_inference(model, val_loader, device)
        test_rows = run_inference(model, test_loader, device)

        if args.disable_thresholding:
            threshold_mode = "disabled"
            threshold_info = None
            val_eval_rows = apply_global_threshold(val_rows, 0.0)
            test_eval_rows = apply_global_threshold(test_rows, 0.0)
        elif args.threshold is not None:
            threshold_mode = "global"
            threshold_info = float(args.threshold)
            val_eval_rows = apply_global_threshold(val_rows, threshold_info)
            test_eval_rows = apply_global_threshold(test_rows, threshold_info)
        else:
            threshold_mode = "class-wise"
            threshold_info, val_threshold_metrics = find_best_classwise_thresholds(val_rows, thresholds)
            val_eval_rows = apply_classwise_thresholds(val_rows, threshold_info)
            test_eval_rows = apply_classwise_thresholds(test_rows, threshold_info)
            print("best validation thresholds (class-wise search):")
            for class_idx, threshold in threshold_info.items():
                print(f"  - {PRED_LABELS[class_idx]}: {threshold:.4f}")
            print(
        ...
        summarize_results(val_eval_rows, threshold_mode=threshold_mode, threshold_info=threshold_info)
        print()
        print("Test summary:")
        summarize_results(test_eval_rows, threshold_mode=threshold_mode, threshold_info=threshold_info)
